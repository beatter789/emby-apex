...
async function select(item: MediaItem): Promise<void> {
  ...
  openDetailDialog();
  if (item.id && tab.value !== 'search') {
    detailLoading.value = true;
    try {
      const response = await getApi<MediaItem>(`/requests/${encodeURIComponent(String(item.id))}`);
      Object.assign(item, response.data || {});
      selected.value = { ...item, ...(response.data || {}) };
      if (selected.value.media_type === 'tv' && !selectedSeasons.value.length) selectedSeasons.value = seasonEntries.value.map(s => s.number);
      return;
    } catch (error) {
  ...
  // Library summaries use a negative snapshot id after retention; the same
  // local detail endpoint resolves those summaries without live MoviePilot.
...
function toggleSeason(season: number): void {
  const opening = !expandedSeasons.value.includes(season);
  ...
  if (opening) void loadSeasonEpisodes(season);
}

async function loadSeasonEpisodes(season: number): Promise<void> {
  const item = selected.value;
  if (!item?.id || item.media_type !== 'tv') return;
  const cached = Array.isArray(item.episodes_info)
    ? item.episodes_info.filter((ep: any) => Number(ep?.season_number ?? ep?.season ?? 1) === season)
    : Array.isArray((item.episodes_info as any)?.[String(season)])
      ? (item.episodes_info as any)[String(season)]
      : [];
  if (cached.length && cached.every((ep: any) => String(ep?.name || ep?.title || '').trim())) return;
  seasonLoading.value[season] = true;
  try {
    const response = await getApi<any[]>(`/requests/${encodeURIComponent(String(item.id))}/season/${season}`);
    const next = { ...(item.episodes_info && !Array.isArray(item.episodes_info) ? item.episodes_info : {}) } as Record<string, any[]>;
    next[String(season)] = response.data || [];
    item.episodes_info = next;
    if (selected.value === item) selected.value = { ...item };
  } catch { /* the season row remains usable when episode loading fails */ }
          ...
          <div class="result-grid portal-poster-grid">
            <article v-for="item in results" :key="recordKey(item)" class="result-card vue-result-card portal-poster-card" role="button" tabindex="0" @click="select(item)" @keydown.enter.prevent="select(item)">
              ...
              <div class="media-card-overlay">
                ...
                <p class="media-card-overview">{{ item.overview || '暂无简介' }}</p>
                <div class="media-card-actions"><button type="button" aria-label="查看详情" @click.stop="select(item)"><Search :size="17" /></button><span aria-hidden="true">♡</span></div>
              </div>
            </article>
          </div>
        </section>
