...
function requestSeasonLabel(value: unknown): string {
  if (!Array.isArray(value)) return '';
  const seasons = value.map((item) => Number(item)).filter((item) => Number.isInteger(item) && item >= 0);
  return [...new Set(seasons)].sort((a, b) => a - b).map((item) => `第 ${item} 季`).join('、');
}
